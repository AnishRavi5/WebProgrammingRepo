...
engine = create_engine("sqlite:///weatherportal.db", echo=True, future=True, connect_args={'check_same_thread': False}, pool_size=20, max_overflow=40)
...
class Admin(Base):
    ...
    # Assignment 5: Modified to not return password in the returned fields
    def as_dict(self):
        fields = {}
...
class ETL():
    ...
    def _load_data(self):
        ...
        # Step 1: fetch city URLs and save to disk
        try:
            dbsession = DBSession()
            cities = dbsession.query(City).all()
            city_names_urls = [(c.name, c.url) for c in cities]
            dbsession.close()
        except Exception as e:
            app.logger.info("ETL: error fetching cities: " + str(e))
            return

        data_dir = os.getcwd() + "/data"
        os.makedirs(data_dir, exist_ok=True)

        for city_name, city_url in city_names_urls:
            try:
                r = requests.get(city_url)
                if r.status_code == 404:
                    app.logger.info("No data available for city: " + city_name + ", skipping.")
                    continue
                fp = open(data_dir + "/" + city_name, "w")
                fp.write(r.text)
                fp.close()
                app.logger.info("ETL: saved data for " + city_name)
            except Exception as e:
                app.logger.info("ETL: error fetching " + city_name + ": " + str(e))

        # Step 2: parse files and store in DB
            ...
            try:
                with open(fPath, "r") as fp:
                    ...
                    for row in reader:
                        ...
                        year = date[0:4]
                        month = date[5:7]
                        for param in weather_parameters_to_track:
                            val = row.get(param, "").strip()
                            if val and val != "":
                                try:
                                    converted = str(int(round(float(val) * 10)))
                                except:
                                    converted = val
                                key = year + "-" + month + "-" + param
                                if key not in param_values:
                                    param_values[key] = converted
            except Exception as e:
                app.logger.info("ETL: error parsing " + filename + ": " + str(e))
                continue
            ...
            try:
                ...
                if city != None:
                    for key, val in param_values.items():
                        ...
                        if exists.count() == 0:
                            weather_params = WeatherParameter(year_month_param=key, values=val, cityId=city.id)
                            dbsession.add(weather_params)
                    dbsession.commit()
                dbsession.close()
            except Exception as e:
                app.logger.info("ETL: error saving data for " + filename + ": " + str(e))
...
def add_admin():
    ...
    if admin != None:
        ...
        # Assignment 5: Use bcrypt to encrypt the password.
...
# Assignment 5: Connected "login-using-google" form to this method
...
def login():
    ...
    if username != '':
        ...
        if users.count() == 0:
            ...
            # Assignment 5: Check that <username, password> exists in the database
            # using bcrypt's checkpw method to verify the encrypted password.
            user = users.first()
    ...
    if 'credentials' in flask.session:
        ...
        # Assignment 5: Insert User in the DB if not already present, leave password empty
        dbsession = DBSession()
